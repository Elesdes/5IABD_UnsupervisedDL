...
class Autoencoder_CNN:
    ...
    def _build_model(self):
        self.encoder = Sequential(name='encoder')
        self.encoder.add(Input(shape=(32, 32, 1)))

        self.encoder.add(Conv2D(32, (3, 3), activation=self.activation, padding='same'))
        self.encoder.add(MaxPooling2D((2, 2), padding='same'))
        self.encoder.add(Conv2D(64, (3, 3), activation=self.activation, padding='same'))
        self.encoder.add(MaxPooling2D((2, 2), padding='same'))
        self.encoder.add(Conv2D(64, (3, 3), activation=self.activation, padding='same'))
        self.encoder.add(MaxPooling2D((2, 2), padding='same'))
        ...
        self.decoder.add(Dense(4 * 4 * 64, activation=self.activation))
        self.decoder.add(Reshape((4, 4, 64)))

        self.decoder.add(Conv2D(64, (3, 3), activation=self.activation, padding='same'))
        self.decoder.add(UpSampling2D((2, 2)))
        self.decoder.add(Conv2D(64, (3, 3), activation=self.activation, padding='same'))
        self.decoder.add(UpSampling2D((2, 2)))
        self.decoder.add(Conv2D(32, (3, 3), activation=self.activation, padding='same'))
        self.decoder.add(UpSampling2D((2, 2)))
